...
# --- Configuration ---
# !!! IMPORTANT: Update these constants to match your specific data file, column names, and events !!!
...
VISIT_DATE_COL = "visit_date"  # Column for the general visit date (for baseline calc)
PARTICIPANT_STATUS_COL = "participant_status"  # For baseline calc mirroring
ENDPOINT_DATE_1_COL = "primary_endpoint_date"  # For baseline calc mirroring
...
EXPECTED_COUNT = len(EXPECTED_EVENT_LIST)  # Expected number of rows per participant
...
print(
    f"Expecting {EXPECTED_COUNT} specific events per participant based on EXPECTED_EVENT_LIST:"
)
...
for i, event in enumerate(EXPECTED_EVENT_LIST):
    if i < 3 or i == len(EXPECTED_EVENT_LIST) - 1:
        ...
        print("  - ...")
print(
    "Checks include: Missing/Duplicate/Unexpected Events, Date/Result presence, "
    "Date Order, Tests Before Baseline."
)
...
try:
    ...
    try:
        ...
        print(" - UnicodeDecodeError with default encoding, trying latin-1...")
...
df[EVENT_NAME_COL] = (
    df[EVENT_NAME_COL].fillna("MISSING_EVENT_NAME").astype(str).str.strip()
)
df[TEST_DATE_COL] = pd.to_datetime(df[TEST_DATE_COL], errors="coerce")
df[VISIT_DATE_COL] = pd.to_datetime(
    df[VISIT_DATE_COL], errors="coerce"
)  # For baseline calc
df[ENDPOINT_DATE_1_COL] = pd.to_datetime(df[ENDPOINT_DATE_1_COL], errors="coerce")
df[PARTICIPANT_STATUS_COL] = pd.to_numeric(
    df[PARTICIPANT_STATUS_COL], errors="coerce"
).astype("Int64")

# Prepare result column
df[RESULT_COL] = df[RESULT_COL].fillna("").astype(str).str.strip()
MISSING_RESULT_VALUES = ["", "nan"]  # Define what constitutes a missing result
...
baseline_event_rows = df_data_rows.loc[
    df_data_rows[EVENT_NAME_COL] == BASELINE_EVENT_NAME
].copy()
baseline_event_rows = baseline_event_rows.sort_values([ID_COL]).drop_duplicates(
    ID_COL, keep="first"
)
...
baseline_event_rows["effective_baseline"] = pd.to_datetime(
    baseline_event_rows["effective_baseline"], errors="coerce"
)
...
if missing_baseline_count > 0:
    print(
        f" - Warning: Effective baseline date could not be determined for "
        f"{missing_baseline_count} rows (likely missing baseline event row or dates). "
        "'Test Before Baseline' check might be affected."
    )
print(" - Effective baseline dates calculated and merged.")
...
for participant_id, group in grouped:
    ...
    effective_baseline_date = (
        effective_baseline_date[0] if len(effective_baseline_date) > 0 else pd.NaT
    )
    ...
    duplicate_events = [
        event for event in EXPECTED_EVENT_SET if event_counts_in_group[event] > 1
    ]
    if duplicate_events:
        participant_issues["duplicate_expected_events"] = sorted(
            [
                f"{event} (Count: {event_counts_in_group[event]})"
    ...
    if unexpected_events:
        ...
        unexpected_events_filtered = [
            e for e in unexpected_events if e != "MISSING_EVENT_NAME"
        ]
        if unexpected_events_filtered:
            participant_issues["unexpected_events_found"] = sorted(
                unexpected_events_filtered
            )
    ...
    if num_expected_event_rows_found != EXPECTED_COUNT:
        participant_issues["incorrect_expected_event_row_count"] = (
            f"Found {num_expected_event_rows_found}, Expected {EXPECTED_COUNT}"
        )

    # --- Data Consistency Checks ---
    # Check A: Date entered without a Result
    date_no_result_rows = group[
        group[TEST_DATE_COL].notna() & group[RESULT_COL].isin(MISSING_RESULT_VALUES)
    ]
    if not date_no_result_rows.empty:
        participant_issues["date_without_result"] = sorted(
            date_no_result_rows[EVENT_NAME_COL].tolist()
        )

    # Check B: Result entered without a Date
    result_no_date_rows = group[
        group[TEST_DATE_COL].isna() & (~group[RESULT_COL].isin(MISSING_RESULT_VALUES))
    ]
    if not result_no_date_rows.empty:
        participant_issues["result_without_date"] = sorted(
            result_no_date_rows[EVENT_NAME_COL].tolist()
        )

    # Check C: Date Order Check (based on EXPECTED_EVENT_LIST order)
    dates_for_order_check = group[
        group[EVENT_NAME_COL].isin(EXPECTED_EVENT_SET) & group[TEST_DATE_COL].notna()
    ].copy()
    if len(dates_for_order_check) > 1:
        dates_for_order_check["event_order"] = dates_for_order_check[
            EVENT_NAME_COL
        ].map(EVENT_ORDER_MAP)
        # Handle cases where an event might not be in the map
        ...
        if not dates_for_order_check[TEST_DATE_COL].is_monotonic_increasing:
            ...
            for i in range(len(dates_series) - 1):
                ...
                if (
                    pd.notna(dates_series.iloc[i])
                    and pd.notna(dates_series.iloc[i + 1])
                    and dates_series.iloc[i] > dates_series.iloc[i + 1]
                ):
            ...
            if out_of_order_details:  # Only add if specific decreases found
                participant_issues["date_order_violation"] = "; ".join(
                    out_of_order_details
                )
    ...
    if pd.notna(
        effective_baseline_date
    ):  # Only perform check if baseline date is valid
        ...
        if not follow_ups_with_date.empty:
            ...
            tests_before_baseline = follow_ups_with_date[
                follow_ups_with_date[TEST_DATE_COL] < effective_baseline_date
            ]
            if not tests_before_baseline.empty:
                participant_issues["test_date_before_baseline"] = sorted(
                    [
                        f"{row[EVENT_NAME_COL]} ({row[TEST_DATE_COL].date()})"
                        for index, row in tests_before_baseline.iterrows()
                    ]
                )
...
if discrepancies:
    ...
    cols_order = [
        "record_id",
        "incorrect_expected_event_row_count",
        "missing_expected_events",
        "duplicate_expected_events",
        "unexpected_events_found",
        "date_without_result",
        "result_without_date",
        "date_order_violation",
        "test_date_before_baseline",
    ]
    ...
    remaining_cols = [
        col for col in discrepancy_df.columns if col not in report_cols_exist
    ]
    discrepancy_df = discrepancy_df[report_cols_exist + remaining_cols]
    ...
    print(discrepancy_df.to_string(index=False, na_rep="-"))
