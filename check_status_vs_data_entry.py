...
"""
Script Purpose:
Identify participants who are recorded with specific 'active' or 'incomplete'
statuses but appear to have no date entries recorded across a defined list of
key study timepoints. This helps flag participants who might need their status
reviewed or updated (e.g., changed to a 'withdrawn without data' or similar status).

Input:
- A CSV data file containing participant data, including participant ID,
  event names, a date column for timepoints, and participant status.

Output:
- Prints lists of participant IDs who meet the criteria (specific status AND
  no dates entered for ALL specified timepoint events).
- IDs are grouped by their original recorded status.

Configuration:
- Users MUST update the constants in the '--- Configuration ---' section below
  to match their specific input file, column names, event names, and the
  participant statuses they wish to check.
"""

import pandas as pd
import numpy as np # Import numpy for np.nan or pd.NA
import sys
...
DATE_COL_TO_CHECK = "test_date"         # The date column to check for missing values across timepoints
PARTICIPANT_STATUS_COL = "participant_status" # Column indicating participant status
...
STATUSES_TO_CHECK = [1, 2, 4] # Example: Statuses for 'Active', 'Completed - Outcome', 'Withdrawn - Specific Reason'
...
POTENTIAL_TARGET_STATUS = 5 # Example: A status code for 'Withdrawn - No Data' or similar
...
try:
    ...
    print(f"  Events Checked      : {len(EVENTS_TO_CHECK_FOR_MISSING_DATE)} specified events (see list in script)")
    ...
    try:
        ...
        print(f"  - UnicodeDecodeError reading '{INPUT_FILENAME}', trying latin-1...")
    ...
    if missing_cols:
        print(f"\nError: Missing required columns in '{INPUT_FILENAME}': {missing_cols}.")
        print("Please ensure the column names in the Configuration section match your file.")
        sys.exit(1)
    ...
    # This assumes the first row accurately reflects the status relevant for this check.
    # If status can change and you need to check based on baseline status, adjust this logic.
    ...
    df_first_rows[PARTICIPANT_STATUS_COL] = pd.to_numeric(df_first_rows[PARTICIPANT_STATUS_COL], errors='coerce')

    # Create a mapping from record_id to participant_status (drop IDs where status is missing)
    record_id_status_map = df_first_rows.set_index(ID_COL)[PARTICIPANT_STATUS_COL].dropna()
    ...
    record_ids_to_check = record_id_status_map[record_id_status_map.isin(STATUSES_TO_CHECK)].index.tolist()
    print(f"\nIdentified {len(record_ids_to_check)} unique Record IDs with status in {STATUSES_TO_CHECK}.")
    print(f"Checking these Record IDs for missing '{DATE_COL_TO_CHECK}' values at ALL specified events...")

    # Filter the main DataFrame to include only rows relevant to the events being checked
    df_events_to_check = df[df[EVENT_NAME_COL].isin(EVENTS_TO_CHECK_FOR_MISSING_DATE)].copy()
    ...
    for record_id in record_ids_to_check:
        ...
        # Check if ALL entries in the specified date column are missing (NaT) for this record_id
        # This check correctly handles participants who might be missing some event rows entirely.
        # If they have *any* non-missing date in the event rows that *are* present, they don't qualify.
        if df_record_events.empty or df_record_events[DATE_COL_TO_CHECK].notna().sum() == 0:
            # If no relevant event rows found OR all dates in existing relevant rows are missing, add the ID
            ...
            if pd.notna(status) and status in potential_candidates_by_status:
                ...
                candidates_found_count +=1 # Count only when added

    print(f"  - Checked {checked_count} participants. Found {candidates_found_count} potential candidates.")

    # --- Report Results ---
    print(f"\n--- Results: Potential Candidates for Status Review ---")
    print(f"(Participants listed below have Status in {STATUSES_TO_CHECK} but NO '{DATE_COL_TO_CHECK}' entries for ANY specified event)")
    ...
    for status_code in sorted(STATUSES_TO_CHECK):
        # Check if the status code exists as a key (it should based on initialization)
        if status_code in potential_candidates_by_status:
            candidates = sorted(potential_candidates_by_status[status_code]) # Sort IDs alphabetically
            count = len(candidates)
            total_potential_candidates += count
            if candidates:
                any_found = True
                print(f"\nRecord IDs with Status {int(status_code)} and EMPTY '{DATE_COL_TO_CHECK}' for ALL specified events ({count} found):")
                ...
                print(f"\nNo Record IDs found with Status {int(status_code)} that have EMPTY '{DATE_COL_TO_CHECK}' for ALL specified events.")
        else:
             print(f"\nStatus code {status_code} was not found among participants checked (this is unexpected).")


    print("\n---------------------------------------------------------------------------------------")
    if any_found:
        print(f"TOTAL potential candidates found across statuses {STATUSES_TO_CHECK}: {total_potential_candidates}")
        print(f"These participants might warrant a status review, potentially changing to Status {POTENTIAL_TARGET_STATUS} or similar based on investigation.")
    else:
        print(f"✅ No participants found matching the criteria (Status {STATUSES_TO_CHECK} and no date entries for specified events).")
    print("---------------------------------------------------------------------------------------")
    ...
    print(f"\nError: A required column defined in the configuration was not found in the file.")
    print(f"Missing column: {e}")
    print("Please ensure the column name constants (e.g., ID_COL, EVENT_NAME_COL) match your file.")
