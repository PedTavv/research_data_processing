...
# --- Configuration ---
# !!! IMPORTANT: Update these constants to match your specific data files, sheet, column names,
# and event names !!!

# Input Files & Sheet
# Replace with paths to your data files
COMPARISON_EXCEL_FILE = (
    "comparison_data_source.xlsx"  # The Excel file name (e.g., master data source)
)
EXCEL_SHEET_NAME = "data_input_sheet"  # The specific sheet name containing the data
...
REPEAT_INSTANCE_COL = (
    "repeat_instance"  # Column indicating repeating instrument instance
)
STATUS_COL = "participant_status"  # Column indicating participant status
ENDPOINT_DATE_1_COL = "primary_endpoint_date"  # Date of primary outcome event/EOS date
ENDPOINT_DATE_2_COL = "secondary_endpoint_date"  # Date of withdrawal/loss to follow-up
VISIT_DATE_COL = "visit_date"  # Column for the general visit date (used for baseline)
TEST_DATE_COL = "test_date"  # Column for the specific date test/assessment performed
ASSESSMENT_COLLECTED_COL = "assessment_collected"  # If assessment data/sample collected
RESULT_COL = "result"  # Column containing a result value (e.g., test score)

# Define the target event names
# !! Update these event names to match those used in your 'EVENT_NAME_COL' !!
TARGET_BASELINE_EVENTS = [
    "study_baseline",
    "study_baseline",
...
# Columns expected on CSV Baseline row AND Excel Baseline row (filtered to no repeat)
...
# Columns expected on CSV Baseline row but Excel EOS row (filtered to no repeat)
# These will be compared CSV Baseline vs Excel EOS (filtered)
CSV_BASELINE_VS_EXCEL_EOS_COLS = [STATUS_COL, ENDPOINT_DATE_1_COL, ENDPOINT_DATE_2_COL]
...
def load_and_clean_data(
    ...
    sheet_name=None,
    id_col=ID_COL,
    event_col=EVENT_NAME_COL,
    repeat_col=REPEAT_INSTANCE_COL,
    all_date_cols=ALL_DATE_COLS,
):
    """Loads and cleans data from CSV or Excel, handling common issues."""
    print(
        f"Loading and cleaning {file_type.upper()} data from: {file_path}"
    ...
    potential_cols = list(
        set(col for col in potential_cols if isinstance(col, str) and col)
    )
    ...
    try:
        if file_type.lower() == "excel":
            ...
            df = pd.read_excel(
                file_path, sheet_name=sheet_name, usecols=lambda c: c in potential_cols
            )
            ...
            try:
                ...
                print(
                    f" - UnicodeDecodeError reading {os.path.basename(file_path)}, "
                    "trying latin-1...",
                    file=sys.stderr,
                )
                df = pd.read_csv(
                    file_path, encoding="latin-1", usecols=lambda c: c in potential_cols
                )
        else:
        ...
        print(
            f"Error loading specified columns or sheet from {os.path.basename(file_path)}: "
            f"{e}",
            file=sys.stderr,
        )
    ...
    if file_type.lower() == "excel":
        core_required.append(repeat_col)  # Repeat instance column essential for Excel
    for col in core_required:
        if col not in df.columns:
            print(
                f"Error: Core required column '{col}' not found after cleaning names in "
...
def perform_cross_source_validation(df_group1_csv, df_group2_csv, df_excel):
    ...
    if df_group1_csv is None or df_group2_csv is None or df_excel is None:
        print(
            "Error: One or more dataframes failed to load. Aborting validation.",
            file=sys.stderr,
        )
        return
    ...
    df_csv_baseline_combined = pd.concat(
        [
            df_group1_csv[
                df_group1_csv[EVENT_NAME_COL].isin(TARGET_BASELINE_EVENTS)
            ].copy(),
            df_group2_csv[
                df_group2_csv[EVENT_NAME_COL].isin(TARGET_BASELINE_EVENTS)
            ].copy(),
        ],
        ignore_index=True,
    )
    df_csv_baseline_filtered = (
        df_csv_baseline_combined.groupby(ID_COL).head(1).reset_index(drop=True)
    )
    ...
    print(
        f" - Filtered Excel Baseline Rows (No Repeat Instance): "
    ...
    print(
        f" - Filtered Excel EOS Rows (No Repeat Instance): {len(df_excel_eos_filtered)}"
    )

    # --- Comparison Logic ---
    print("--- Comparing data points across sources ---")
    discrepancies = []
    csv_baseline_ids = set(df_csv_baseline_filtered[ID_COL].unique())

    # Iterate through unique IDs found in the filtered CSV baseline data
    for record_id in csv_baseline_ids:
        csv_baseline_row = df_csv_baseline_filtered[
            df_csv_baseline_filtered[ID_COL] == record_id
        ].iloc[0]
        ...
        excel_baseline_row = (
            excel_baseline_row_match.iloc[0]
            if excel_baseline_exists
            else pd.Series(dtype="object")
        )
        excel_eos_row = (
            excel_eos_row_match.iloc[0]
            if excel_eos_exists
            else pd.Series(dtype="object")
        )
        ...
        def check_mismatch(val1, val2, col_name):
            ...
            # Rule 3: Both present -> Must be equal
                ...
                if col_name == RESULT_COL:
                    ...
                    if int1 != int2:
                        note_msg = (
                            f"Integer part differs: CSV={int1}, Excel={int2} "
                            f"(Originals: '{val1}', '{val2}')"
                        )
                    ...
                    if str1 != str2:
                        note_msg = (
                            f"Status values differ (Originals: '{val1}', '{val2}')"
                        )
                        return True, note_msg
        ...
        for col in BASELINE_CSV_VS_EXCEL_BASE_COLS:
            if col not in csv_baseline_row.index or (
                excel_baseline_exists and col not in excel_baseline_row.index
            ):
                print(
                    f"Warning: Column '{col}' missing during comparison for ID "
                    f"{record_id}. Check data sources.",
                    file=sys.stderr,
                )
                ...
                discrepancies.append(
                    {
                        ...
                        "CSV_value": (
                            csv_val.strftime("%Y-%m-%d")
                            if isinstance(csv_val, pd.Timestamp)
                            else csv_val
                        ),
                        "Excel_value": (
                            excel_val.strftime("%Y-%m-%d")
        ...
        for col in CSV_BASELINE_VS_EXCEL_EOS_COLS:
            if col not in csv_baseline_row.index or (
                excel_eos_exists and col not in excel_eos_row.index
            ):
                print(
                    f"Warning: Column '{col}' missing during comparison for ID "
                    f"{record_id}. Check data sources.",
    ...
    for record_id in ids_excel_base_only:
        ...
        discrepancies.append(
            {
                "record_id": record_id,
                "field": "Record Existence",
                "Source_CSV_Row": "N/A",
                "Source_Excel_Row": "Baseline (Filtered)",
                "CSV_value": "Missing in Filtered CSV Baseline",
                "Excel_value": f"Exists ({excel_row.get(EVENT_NAME_COL, 'Event N/A')})",
                "note": "Record found in filtered Excel Baseline but not filtered CSV Baseline.",
            }
        )

    ids_excel_eos_only = list(excel_eos_ids - csv_baseline_ids - excel_baseline_ids)
        ...
        discrepancies.append(
            {
                ...
                "Excel_value": f"Exists ({excel_row.get(EVENT_NAME_COL, 'Event N/A')})",
                "note": (
                    "Record found in filtered Excel EOS but not filtered CSV Baseline "
                    "(and not caught as Excel Baseline only)."
                ),
            }
        )
    ...
    csv_arm1_ids = set(
        df_csv_baseline_filtered[
            df_csv_baseline_filtered[EVENT_NAME_COL].isin(arm1_baseline_events)
        ][ID_COL]
    )
    csv_arm2_ids = set(
        df_csv_baseline_filtered[
            df_csv_baseline_filtered[EVENT_NAME_COL].isin(arm2_baseline_events)
        ][ID_COL]
    )
    excel_arm1_ids = set(
        df_excel_eos_filtered[
            df_excel_eos_filtered[EVENT_NAME_COL].isin(arm1_eos_events)
        ][ID_COL]
    )
    excel_arm2_ids = set(
        df_excel_eos_filtered[
            df_excel_eos_filtered[EVENT_NAME_COL].isin(arm2_eos_events)
        ][ID_COL]
    )
    ...
    for pid in common_ids:
        ...
        if csv_is_arm1 != excel_is_arm1:
            csv_event = df_csv_baseline_filtered.loc[
                df_csv_baseline_filtered[ID_COL] == pid, EVENT_NAME_COL
            ].iloc[0]
            excel_event = df_excel_eos_filtered.loc[
                df_excel_eos_filtered[ID_COL] == pid, EVENT_NAME_COL
            ].iloc[0]
            arm_mismatches.append(
                {
                    "record_id": pid,
                    "issue": (
                        f"Arm Mismatch: CSV Baseline Event '{csv_event}' vs "
                        f"Excel EOS Event '{excel_event}'"
                    ),
                }
            )

    print(
        f"--- Comparison checks finished. Found {len(discrepancies)} value/existence "
        f"discrepancies and {len(arm_mismatches)} potential arm mismatches. ---"
    )
    ...
    def print_status_counts(df, description):
        ...
        if STATUS_COL in df.columns:
            status_counts = (
                df[STATUS_COL].dropna().astype(int).value_counts().sort_index()
            )
            ...
            if total_with_status > 0:
                ...
                if missing_status > 0:
                    print(f"    Status Missing/Invalid: {missing_status}")
            else:
                print(f"    No valid '{STATUS_COL}' values found.")
        else:
            print(f"  '{STATUS_COL}' column not found.")
    ...
    print_status_counts(
        df_excel_eos_filtered[df_excel_eos_filtered[ID_COL].isin(excel_arm1_ids)],
        "Filtered Excel EOS - Arm 1",
    )
    print_status_counts(
        df_excel_eos_filtered[df_excel_eos_filtered[ID_COL].isin(excel_arm2_ids)],
        "Filtered Excel EOS - Arm 2",
    )
    ...
    if not discrepancies and not arm_mismatches:
        print(
            "✅ No value/existence discrepancies or arm mismatches found based on "
            "comparison criteria."
        )
    else:
    ...
    if discrepancies:
        ...
        try:
            discrepancies_df.to_csv(
                DISCREPANCY_VALUE_FILE, index=False, encoding="utf-8"
            )
...
if __name__ == "__main__":
    ...
    df_excel = load_and_clean_data(
        COMPARISON_EXCEL_FILE, file_type="excel", sheet_name=EXCEL_SHEET_NAME
    )
