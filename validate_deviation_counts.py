...
# --- Configuration ---
# !!! IMPORTANT: Update these constants to match the main calculation script and output file !!!
# File with pre-calculated counts to validate (output of main calculation script)
# Example: Output from calculate_test_deviations.py
INPUT_FILE_TO_VALIDATE = "participant_data_processed.csv"
...
# e.g., Date of event, or end-of-study date for some statuses
...
OUTSIDE_WINDOW_COUNT_COL = (
    "outside_window_test_count"  # Count of performed tests outside the grace period
)
...
print(
    f"Using Proxy Baseline: '{VISIT_DATE_COL}' from baseline event (if '{TEST_DATE_COL}' is missing)"
)
...
try:
    ...
    try:
        ...
        try:
            df = pd.read_csv(
                INPUT_FILE_TO_VALIDATE, encoding="latin1", low_memory=False
            )
            print(" - Loaded using latin1 encoding.")
        except UnicodeDecodeError:
            df = pd.read_csv(
                INPUT_FILE_TO_VALIDATE, encoding="cp1252", low_memory=False
            )
...
df[MISSED_TEST_COUNT_COL] = pd.to_numeric(
    df[MISSED_TEST_COUNT_COL], errors="coerce"
).astype("Int64")
df[OUTSIDE_WINDOW_COUNT_COL] = pd.to_numeric(
    df[OUTSIDE_WINDOW_COUNT_COL], errors="coerce"
).astype("Int64")
df[TOTAL_DEVIATION_COUNT_COL] = pd.to_numeric(
    df[TOTAL_DEVIATION_COUNT_COL], errors="coerce"
).astype("Int64")
...
baseline_event_rows = df.loc[
    (df[EVENT_NAME_COL] == BASELINE_EVENT_NAME) & (df[ID_COL] != "")
].copy()
...
if (
    ...
    print("Error: Missing date columns for baseline calculation during validation.")
    sys.exit(1)
...
endpoint_cols_exist = [
    col for col in [ENDPOINT_DATE_1_COL, ENDPOINT_DATE_2_COL] if col in df.columns
]
pinfo_raw = (
    df.loc[
        (df[ID_COL] != "") & (df[EVENT_NAME_COL] == BASELINE_EVENT_NAME),
...
participants_for_recalc = participants_for_recalc[
    participants_for_recalc[PARTICIPANT_STATUS_COL].isin(STATUSES_TO_RECALCULATE)
].copy()
print(
    f"Recalculating counts for {len(participants_for_recalc)} participants "
    f"(Statuses {STATUSES_TO_RECALCULATE} with baseline)..."
)
...
performed_tests_df = performed_tests_df.sort_values(
    [ID_COL, EVENT_NAME_COL, TEST_DATE_COL]
).drop_duplicates(subset=[ID_COL, EVENT_NAME_COL], keep="first")
performed_dict = {
    (str(row[ID_COL]), row[EVENT_NAME_COL]): row[TEST_DATE_COL]
...
if not participants_for_recalc.empty:
    for _, participant_row in participants_for_recalc.iterrows():
        ...
        if status in [2, 3, 4] and pd.isna(participant_end_point_date):
            continue  # Skip if needed endpoint date missing
        if status == 1 and pd.isna(base_dt):
            continue  # Skip if baseline missing for status 1

        # Check Baseline Test
        baseline_scheduled_date = base_dt
        baseline_is_expected = (status == 1) or (
            pd.notna(participant_end_point_date)
            and baseline_scheduled_date <= participant_end_point_date
        )
        ...
        for month_offset, event_name in EXPECTED_FOLLOWUP_SCHEDULE.items():
            ...
            if status == 1 and pd.notna(base_dt):
                is_expected = (
                    True  # Assumes all defined events are expected for Status 1
                )
...
print(
    f"Comparing recalculated values with stored values for Statuses {STATUSES_TO_RECALCULATE}..."
)
if not recalc_results_list:
    print(
        "Warning: No results were recalculated. "
        f"Cannot perform comparison for Statuses {STATUSES_TO_RECALCULATE}."
    )
    ...
    df_stored_counts = df.loc[
        (df[EVENT_NAME_COL] == BASELINE_EVENT_NAME) & (df[ID_COL] != "")
    ].copy()
    # Ensure unique baseline row per ID if duplicates somehow exist
    df_stored_counts = df_stored_counts.sort_values([ID_COL]).drop_duplicates(
        ID_COL, keep="first"
    )
    # Select only relevant columns
    df_stored_counts = df_stored_counts[
        [
            ID_COL,
            PARTICIPANT_STATUS_COL,
            MISSED_TEST_COUNT_COL,
    ...
    def compare_values(v1, v2):
        ...
        if v1_isna or v2_isna:
            return False  # One missing, one not is a mismatch
        return v1 == v2  # Both have values, compare them

    comparison_df["missed_match"] = comparison_df.apply(
        lambda row: compare_values(
            row[MISSED_TEST_COUNT_COL], row["recalc_" + MISSED_TEST_COUNT_COL]
        ),
        axis=1,
    )
    comparison_df["outside_window_match"] = comparison_df.apply(
        lambda row: compare_values(
            row[OUTSIDE_WINDOW_COUNT_COL], row["recalc_" + OUTSIDE_WINDOW_COUNT_COL]
        ),
        axis=1,
    )

    # Recalculate total based on the RECALCULATED components
    comparison_df["recalc_" + TOTAL_DEVIATION_COUNT_COL] = comparison_df[
        "recalc_" + MISSED_TEST_COUNT_COL
    ].fillna(0) + comparison_df["recalc_" + OUTSIDE_WINDOW_COUNT_COL].fillna(0)
    comparison_df["total_match"] = comparison_df.apply(
        lambda row: compare_values(
            row[TOTAL_DEVIATION_COUNT_COL], row["recalc_" + TOTAL_DEVIATION_COUNT_COL]
        ),
        axis=1,
    )
...
)  # Handle empty comparison_df case
...
if num_discrepancies > 0:
    ...
    cols_to_display = [
        ...
        "recalc_" + MISSED_TEST_COUNT_COL,
        OUTSIDE_WINDOW_COUNT_COL,
        "recalc_" + OUTSIDE_WINDOW_COUNT_COL,
        TOTAL_DEVIATION_COUNT_COL,
        "recalc_" + TOTAL_DEVIATION_COUNT_COL,
    ]
    # Ensure columns exist before trying to display
    cols_to_display_exist = [
        col for col in cols_to_display if col in discrepancies.columns
    ]
    ...
    try:
        ...
        discrepancies[cols_to_display_exist].to_csv(
            output_filename, index=False, encoding="utf-8"
        )
    ...
    print(
        f"\n✅ No discrepancies found for Statuses {STATUSES_TO_RECALCULATE}. "
        "All recalculated values match stored values."
    )
...
if PARTICIPANT_STATUS_COL in df.columns:
    ...
    status_5_baseline_ids = set(
        baseline_rows.loc[
            baseline_rows[PARTICIPANT_STATUS_COL] == STATUS_EXPECTING_BLANK, ID_COL
        ]
        .astype(str)
        .unique()
    )
    ...
    print(
        f"Warning: Status column '{PARTICIPANT_STATUS_COL}' not found. "
        f"Cannot identify Status {STATUS_EXPECTING_BLANK} participants."
    )

print(
    f" - Found {len(status_5_baseline_ids)} unique participants with status "
    f"{STATUS_EXPECTING_BLANK} on their baseline row."
)
...
if status_5_baseline_ids:
    ...
    cols_exist_for_check = [
        col for col in cols_to_check_blank if col in status_5_all_rows.columns
    ]

    if cols_exist_for_check:
        # Find rows where ANY of the count columns are NOT NA
        status_5_with_values = status_5_all_rows[
            status_5_all_rows[cols_exist_for_check].notna().any(axis=1)
        ].copy()
        num_status_5_errors = len(
            status_5_with_values[ID_COL].unique()
        )  # Count unique IDs with errors

        print(
            f"Number of Status {STATUS_EXPECTING_BLANK} participants with non-blank values "
            "in count columns (across ANY of their rows): {num_status_5_errors}"
        )

        if num_status_5_errors > 0:
            print(
                f"\nDetails of Status {STATUS_EXPECTING_BLANK} Participants "
                "with Unexpected Values:"
            )
            # Display the specific rows that have the non-blank values
            print(
                status_5_with_values[
                    [ID_COL, EVENT_NAME_COL, PARTICIPANT_STATUS_COL]
                    + cols_exist_for_check
                ].to_string(index=False)
            )
            # Save these specific error rows
            status_5_errors = status_5_with_values[
                [ID_COL, EVENT_NAME_COL, PARTICIPANT_STATUS_COL] + cols_exist_for_check
            ].to_dict("records")
        else:
            print(
                f"✅ All Status {STATUS_EXPECTING_BLANK} participants correctly "
                "have blank values in count columns."
            )
    else:
        print(
            " - Count columns not found to check for Status {STATUS_EXPECTING_BLANK}."
        )
else:
...
if status_5_errors:
    try:
        ...
        pd.DataFrame(status_5_errors).to_csv(
            output_filename_s5, index=False, encoding="utf-8"
        )
        print(
            f"\nStatus {STATUS_EXPECTING_BLANK} error details saved to: "
            f"{output_filename_s5}"
        )
    except Exception as e:
        print(
            f"\nError saving Status {STATUS_EXPECTING_BLANK} errors to CSV: {e}",
            file=sys.stderr,
        )
