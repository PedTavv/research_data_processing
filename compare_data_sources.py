...
COMPARISON_EXCEL_FILE = (
    "comparison_data.xlsx"  # e.g., Master data or alternative source for comparison
)
EXCEL_SHEET_NAME = (
    "data_sheet"  # The sheet name within the Excel file containing the data
)
...
# Example: Column indicating if a specific assessment/test was completed
ASSESSMENT_COMPLETE_COL = "assessment_complete"
# Example: Column indicating if assessment data/sample was collected
ASSESSMENT_COLLECTED_COL = "assessment_collected"
RESULT_COL = (
    "result"  # Column containing a result value (e.g., test score, measurement)
)
TEST_DATE_COL = (
    "test_date"  # Column for the specific date the test/assessment was performed
)
# Column indicating participant status (e.g., active, withdrawn)
PARTICIPANT_STATUS_COL = "participant_status"
# e.g., Date of primary outcome event or EOS date for some statuses
ENDPOINT_DATE_1_COL = "primary_endpoint_date"
# e.g., Date of withdrawal or loss to follow-up for other statuses
ENDPOINT_DATE_2_COL = "secondary_endpoint_date"
# Column indicating repeating instrument instance (often relevant in exports)
REPEAT_INSTANCE_COL = "repeat_instance"
...
BASELINE_EVENT_NAMES = [
    "study_baseline",
    "study_baseline",
]  # List of possible baseline event names
EOS_EVENT_NAMES = [
    "end_of_study_visit",
    "end_of_study_visit",
]  # List of possible End-of-Study event names

# --- Comparison Columns Configuration ---
# Columns expected on the baseline row in BOTH CSV and Excel
# (when REPEAT_INSTANCE_COL is empty in Excel)
# These will be compared: CSV Baseline Row vs Excel Baseline Row (Filtered)
# Note: ASSESSMENT_COMPLETE_COL was intentionally excluded from this list
# based on original script comment.
...
# Columns expected on CSV Baseline row but compared against the Excel EOS row
# (when REPEAT_INSTANCE_COL is empty in Excel)
...
def compare_data_sources(
    ...
    try:
        ...
        try:
            ...
            print(
                "Warning: Failed to read CSV with latin-1 encoding. Trying utf-8.",
                file=sys.stderr,
            )
    ...
    missing_csv_cols = [
        ...
        if col not in df_csv_combined.columns and col != REPEAT_INSTANCE_COL
    ]  # Allow REPEAT_INSTANCE_COL to be missing in CSV
    missing_excel_cols = [
        col for col in all_required_cols if col not in df_excel.columns
    ]

    if missing_csv_cols:
        print(
            f"Error: Required columns missing in combined CSV data: {missing_csv_cols}",
            file=sys.stderr,
        )
        sys.exit(1)
    if missing_excel_cols:
        msg = (
            f"Error: Required columns missing in {excel_path} (sheet '{sheet_name}'): "
        )
        print(msg + f"{missing_excel_cols}", file=sys.stderr)
        sys.exit(1)
    ...
    df_group1_baseline = (
        df_group1[df_group1[EVENT_NAME_COL].isin(baseline_event_names)]
        .groupby(ID_COL)
        .head(1)
        .reset_index(drop=True)
    )
    df_group2_baseline = (
        df_group2[df_group2[EVENT_NAME_COL].isin(baseline_event_names)]
        .groupby(ID_COL)
        .head(1)
        .reset_index(drop=True)
    )
    ...
    df_csv_baseline_filtered = (
        df_csv_combined[df_csv_combined[EVENT_NAME_COL].isin(baseline_event_names)]
        .groupby(ID_COL)
        .head(1)
        .reset_index(drop=True)
    )
    ...
    df_excel_baseline_filtered = (
        df_excel[
            (df_excel[EVENT_NAME_COL].isin(baseline_event_names))
            & (
                df_excel[REPEAT_INSTANCE_COL].isnull()
                | (df_excel[REPEAT_INSTANCE_COL] == "")
    ...
    print(
        f" - Filtered Excel baseline rows (empty repeat instance): "
        f"{len(df_excel_baseline_filtered)}"
    )

    # Filter Excel for EOS events AND where repeat instance is null/empty
    df_excel_eos_filtered = (
        df_excel[
    ...
    print(
        f" - Filtered Excel EOS rows (empty repeat instance): "
        f"{len(df_excel_eos_filtered)}"
    )
    ...
    csv_baseline_ids = set(
        df_csv_baseline_filtered[ID_COL].unique()
    ...
    for index, csv_baseline_row in df_csv_baseline_filtered.iterrows():
        ...
        excel_baseline_row_match = df_excel_baseline_filtered[
            df_excel_baseline_filtered[ID_COL] == record_id
        ]

        # Find the corresponding EOS row in the filtered Excel data
        excel_eos_row_match = df_excel_eos_filtered[
            df_excel_eos_filtered[ID_COL] == record_id
        ]
            ...
            discrepancies.append(
                {
                    ...
                    "note": (
                        "Record ID exists in CSV baseline but relevant baseline row "
                        "is missing in filtered Excel."
                    ),
                }
            )
        else:
            excel_baseline_row = excel_baseline_row_match.iloc[
                0
            ...
            for col in base_compare_cols:
                ...
                if col == RESULT_COL:
                    ...
                    if csv_result_int != excel_result_int:
                        is_discrepant = True
                        note = (
                            f"Integer part differs: CSV={csv_result_int}, "
                            f"Excel={excel_result_int}"
                        )
                elif col in date_cols_to_normalize:
                    if not (pd.isna(csv_value) and pd.isna(excel_value)) and (
                        csv_value != excel_value
                    ):
                        is_discrepant = True
                        note = "Dates differ"
                        csv_value = (
                            csv_value.strftime("%Y-%m-%d")
                    ...
                    if not (pd.isna(csv_value) and pd.isna(excel_value)) and (
                        str(csv_value).strip() != str(excel_value).strip()
                    ):
                        is_discrepant = True
        ...
        if excel_eos_row_match.empty:
            ...
            has_relevant_csv_data = any(
                pd.notna(csv_baseline_row.get(col)) for col in eos_compare_cols
            )
                ...
                discrepancies.append(
                    {
                        ...
                        "CSV_value": (
                            "Relevant data exists in CSV baseline for EOS fields"
                        ),
                        "Excel_value": "Relevant EOS Row Missing in Excel (filtered)",
                        "note": (
                            f"CSV baseline has data for EOS fields ({eos_compare_cols}), "
                            "but relevant EOS row missing in filtered Excel."
                        ),
                    }
                )
        else:
            excel_eos_row = excel_eos_row_match.iloc[
                0
            ]  # Get the first matching relevant EOS row
            ...
            for col in eos_compare_cols:
                ...
                if col in [ENDPOINT_DATE_1_COL, ENDPOINT_DATE_2_COL]:
                    ...
                    csv_comp = (
                        str(csv_value).split(".")[0].strip()
                        if pd.notna(csv_value)
                        else "NA"
                    )
                    excel_comp = (
                        str(excel_value).split(".")[0].strip()
                        if pd.notna(excel_value)
                        else "NA"
                    )
                    if not (pd.isna(csv_value) and pd.isna(excel_value)) and (
    ...
    if excel_ids_missing_in_csv_baseline:
        print(
            f" - Found {len(excel_ids_missing_in_csv_baseline)} Record IDs in Excel "
            "Baseline (filtered) but not in CSV Baseline."
        )
        for record_id in excel_ids_missing_in_csv_baseline:
            excel_row = df_excel_baseline_filtered[
                df_excel_baseline_filtered[ID_COL] == record_id
            ].iloc[0]
            discrepancies.append(
                {
                    ...
                    "note": (
                        "Record ID exists in filtered Excel baseline but is missing "
                        "in CSV baseline data."
                    ),
                }
            )
    ...
    if excel_eos_ids_not_in_csv_baseline:
        print(
            f" - Found {len(excel_eos_ids_not_in_csv_baseline)} Record IDs in Excel "
            "EOS (filtered) but not in CSV Baseline."
        )
        for record_id in excel_eos_ids_not_in_csv_baseline:
            excel_row = df_excel_eos_filtered[
                df_excel_eos_filtered[ID_COL] == record_id
            ].iloc[0]
            discrepancies.append(
                {
                    ...
                    "note": (
                        "Relevant EOS row exists in filtered Excel but corresponding "
                        "baseline row is missing in CSV."
                    ),
                }
            )

    print(
        f"--- Comparison Complete. Found {len(discrepancies)} total discrepancies. ---"
    )

    # --- Report Summary ---
    print("\n=== SUMMARY OF DATA COMPARISON (CSV vs Excel) ===")
    print(
        f"Total unique record IDs with relevant baseline event in combined CSVs: "
        f"{len(csv_baseline_ids)}"
    )
    print(
        f"Total unique record IDs with relevant baseline event in filtered Excel: "
        f"{len(excel_baseline_ids)}"
    )
    print(
        f"Total unique record IDs with relevant EOS event in filtered Excel: "
        f"{len(excel_eos_ids)}"
    )
    ...
    arm2_baseline_event = (
        baseline_event_names[1] if len(baseline_event_names) > 1 else None
    )
    ...
    if arm1_baseline_event:
        arm1_count_csv = len(
            df_csv_baseline_filtered[
                df_csv_baseline_filtered[EVENT_NAME_COL] == arm1_baseline_event
            ]
        )
    ...
    if arm2_baseline_event:
        arm2_count_csv = len(
            df_csv_baseline_filtered[
                df_csv_baseline_filtered[EVENT_NAME_COL] == arm2_baseline_event
            ]
        )
        print(f"    Arm 2: {arm2_count_csv}")
    print(
        f"    Total Records with Relevant Baseline Event: "
        f"{arm1_count_csv + arm2_count_csv}"
    )

    print("\n  Participant Status Counts and Percentages:")
    if PARTICIPANT_STATUS_COL in df_csv_baseline_filtered.columns:
        status_counts_csv = (
            df_csv_baseline_filtered[PARTICIPANT_STATUS_COL]
            .dropna()
            .astype(int)
            .value_counts()
            .sort_index()
        ...
        if total_with_status_csv > 0:
            ...
            missing_status_csv = (
                df_csv_baseline_filtered[PARTICIPANT_STATUS_COL].isnull().sum()
            )
            if missing_status_csv > 0:
                print(f"    Status Missing/Invalid: {missing_status_csv}")
        else:
            print(f"    No valid '{PARTICIPANT_STATUS_COL}' values found.")
    else:
        print(f"  '{PARTICIPANT_STATUS_COL}' column not found.")
    ...
    if arm1_eos_event:
        arm1_count_excel_eos = len(
            df_excel_eos_filtered[
                df_excel_eos_filtered[EVENT_NAME_COL] == arm1_eos_event
            ]
        )
        print(f"    Arm 1: {arm1_count_excel_eos}")
    if arm2_eos_event:
        arm2_count_excel_eos = len(
            df_excel_eos_filtered[
                df_excel_eos_filtered[EVENT_NAME_COL] == arm2_eos_event
            ]
        )
        print(f"    Arm 2: {arm2_count_excel_eos}")
    print(
        f"    Total Records with Relevant EOS Event: "
        f"{arm1_count_excel_eos + arm2_count_excel_eos}"
    )
    ...
    print("    Arm 1:")
    if arm1_eos_event:
        df_excel_eos_arm1 = df_excel_eos_filtered[
            df_excel_eos_filtered[EVENT_NAME_COL] == arm1_eos_event
        ].copy()
        if PARTICIPANT_STATUS_COL in df_excel_eos_arm1.columns:
            status_counts_excel_arm1 = (
                df_excel_eos_arm1[PARTICIPANT_STATUS_COL]
                .dropna()
                .astype(int)
                .value_counts()
                .sort_index()
            ...
            if total_with_status_excel_arm1 > 0:
                for status, count in status_counts_excel_arm1.items():
                    ...
                    print(f"      Status {status}: {count} ({percentage:.2f}%)")
                missing_status_excel_arm1 = (
                    df_excel_eos_arm1[PARTICIPANT_STATUS_COL].isnull().sum()
                )
                if missing_status_excel_arm1 > 0:
                    print(f"      Status Missing/Invalid: {missing_status_excel_arm1}")
            else:
                print("      No valid status values found for Arm 1.")
        else:
            print(f"    '{PARTICIPANT_STATUS_COL}' column not found for Arm 1.")
    else:
        print("    Arm 1 EOS event not defined.")

    # Arm 2 (Excel EOS)
    print("\n    Arm 2:")
    if arm2_eos_event:
        df_excel_eos_arm2 = df_excel_eos_filtered[
            df_excel_eos_filtered[EVENT_NAME_COL] == arm2_eos_event
        ].copy()
        if PARTICIPANT_STATUS_COL in df_excel_eos_arm2.columns:
            status_counts_excel_arm2 = (
                df_excel_eos_arm2[PARTICIPANT_STATUS_COL]
                .dropna()
                .astype(int)
                .value_counts()
                .sort_index()
            ...
            if total_with_status_excel_arm2 > 0:
                for status, count in status_counts_excel_arm2.items():
                    ...
                    print(f"      Status {status}: {count} ({percentage:.2f}%)")
                missing_status_excel_arm2 = (
                    df_excel_eos_arm2[PARTICIPANT_STATUS_COL].isnull().sum()
                )
                if missing_status_excel_arm2 > 0:
                    print(f"      Status Missing/Invalid: {missing_status_excel_arm2}")
            else:
                print("      No valid status values found for Arm 2.")
        else:
            print(f"    '{PARTICIPANT_STATUS_COL}' column not found for Arm 2.")
    else:
        print("    Arm 2 EOS event not defined.")
    ...
    if PARTICIPANT_STATUS_COL in df_group1_baseline.columns:
        status_counts_g1_csv = (
            df_group1_baseline[PARTICIPANT_STATUS_COL]
            .dropna()
            .astype(int)
            .value_counts()
            .sort_index()
        ...
        if total_with_status_g1_csv > 0:
            for status, count in status_counts_g1_csv.items():
                ...
                print(f"    Status {status}: {count} ({percentage:.2f}%)")
            missing_status_g1_csv = (
                df_group1_baseline[PARTICIPANT_STATUS_COL].isnull().sum()
            )
            if missing_status_g1_csv > 0:
                print(f"    Status Missing/Invalid: {missing_status_g1_csv}")
        else:
            print(f"    No valid '{PARTICIPANT_STATUS_COL}' values found.")
    else:
        print(f"  '{PARTICIPANT_STATUS_COL}' column not found.")
    ...
    if PARTICIPANT_STATUS_COL in df_group2_baseline.columns:
        status_counts_g2_csv = (
            df_group2_baseline[PARTICIPANT_STATUS_COL]
            .dropna()
            .astype(int)
            .value_counts()
            .sort_index()
        ...
        if total_with_status_g2_csv > 0:
            for status, count in status_counts_g2_csv.items():
                ...
                print(f"    Status {status}: {count} ({percentage:.2f}%)")
            missing_status_g2_csv = (
                df_group2_baseline[PARTICIPANT_STATUS_COL].isnull().sum()
            )
            if missing_status_g2_csv > 0:
                print(f"    Status Missing/Invalid: {missing_status_g2_csv}")
        else:
            print(f"    No valid '{PARTICIPANT_STATUS_COL}' values found.")
    else:
        print(f"  '{PARTICIPANT_STATUS_COL}' column not found.")
    ...
    if discrepancies:
        ...
        discrepancies_df = discrepancies_df[
            [col for col in discrepancy_cols_order if col in discrepancies_df.columns]
        ]
        ...
        try:
            discrepancies_df.to_csv(
                DISCREPANCY_OUTPUT_FILE, index=False, encoding="utf-8"
            )
    ...
    if (
        ...
        csv1_excel2 = list(
            (csv_arm1_ids & excel_arm2_ids)
            - (csv_arm1_ids & excel_arm1_ids)
            - (csv_arm2_ids & excel_arm2_ids)
        )
        csv2_excel1 = list(
            (csv_arm2_ids & excel_arm1_ids)
            - (csv_arm1_ids & excel_arm1_ids)
            - (csv_arm2_ids & excel_arm2_ids)
        )
        ...
        if csv1_excel2 or csv2_excel1:
            ...
            if csv1_excel2:
                print(
                    f"  Record ID(s) appearing as Arm 1 in CSV Baseline but Arm 2 in Excel EOS: {', '.join(csv1_excel2)}"
                )
            if csv2_excel1:
                print(
                    f"  Record ID(s) appearing as Arm 2 in CSV Baseline but Arm 1 in Excel EOS: {', '.join(csv2_excel1)}"
                )
        else:
            print(
